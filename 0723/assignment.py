...
for episode in range(1000):
    ...
    for t in range(10000):
        ...
        # Update Q-table
        target_Q = reward if done else reward + env.gamma * (Q[next_state,next_action])
        delta = target_Q - Q[state, action]
        e[state, action] += 1
        ###################
        # TODO: 아래 pass 부분을 수정하여 SARSA(\lambda)를 구현해 봅시다.
        # Q[state, action] = Q[state, action] + alpha * delta  # 이건 일반 SARSA
